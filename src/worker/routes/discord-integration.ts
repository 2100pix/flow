...
// Konstanta tipe dan opsi API Discord
...
// Helper: cek zona waktu IANA valid
...
// Routes API integrasi Discord
export const discordIntegrationRoutes = new Hono<DiscordIntegrationEnv>();

// Helper: arahkan balik ke settings + status OAuth
...
// Daftarkan ulang slash command ke server Discord
...
// Mulai OAuth (redirect ke Discord)
...
// Callback OAuth: simpan token dan guild terhubung
 ...
 * workspace ini:
 * - forum belum ready sehingga dikirim ulang
 *   provisioning-nya (termasuk pemulihan
 *   kanal yang terlanjur dibuat)
 * - forum sudah ready sehingga cukup disinkronkan
 *   overwrites-nya
 *
...
discordIntegrationRoutes.post("/resync-forums", requireAuth, requirePermission("settings.manage"), async (c) => {
  ...
  for (const target of targets) {
     ...
     * pasangan yang sama lalu tabel outbox punya
     * unique index pada (event_type, aggregate_id).
     * Pola yang sama dengan task_thread.sync
     */
    const deletePrevious = db.delete(discordOutboxEvents).where(and(eq(discordOutboxEvents.aggregateId, target.projectId), eq(discordOutboxEvents.eventType, eventType), eq(discordOutboxEvents.status, "pending")));
...
discordIntegrationRoutes.patch(
  ...
  async (c) => {
    ...
    const syncTargets = await db
      ...
      .where(and(eq(projects.workspaceId, auth.workspace.id), eq(projects.visibility, "workspace"), isNull(projects.archivedAt), eq(projectDiscordForums.provisioningStatus, "ready")));
...
// Perbarui pengaturan reminder (jam dan zona waktu)
discordIntegrationRoutes.patch(
  "/reminders",
  requireAuth,
  requirePermission("settings.manage"),
...
// Nyalakan/mati sinkronisasi Discord
discordIntegrationRoutes.patch(
  "/",
  requireAuth,
  requirePermission("settings.manage"),
...
// Ambil status integrasi Discord saat ini
